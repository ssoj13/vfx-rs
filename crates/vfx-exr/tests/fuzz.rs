...
use vfx_exr::image::pixel_vec::PixelVec;
use vfx_exr::image::read::read_first_rgba_layer_from_file;
use vfx_exr::prelude::*;
use std::ffi::OsStr;
use std::fs::File;
...
#[test]
pub fn fuzzed() {
    for ref file in exr_files("../../test/assets-exr/fuzzed", false) {
        let _ = read()
            .no_deep_data()
...
pub fn damaged() {
    ...
    for ref file in exr_files("../../test/assets-exr/invalid", false) {
...
pub fn fuzz() {
    ...
    let files: Vec<PathBuf> = exr_files("../../test/assets-exr", true).collect();
    ...
    let mut records = File::create("../../test/assets-exr/fuzzed/list.txt").unwrap();
    records
        ...
        if fuzz_index >= start_index {
            ...
            if let Err(_) = result {
                ...
                let mut saved = File::create(format!(
                    "../../test/assets-exr/fuzzed/fuzz_{}_{}.exr",
                    fuzz_index, seed
                ))
                .unwrap();
