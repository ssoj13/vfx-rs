...
use crate::image::recursive::{NoneMore, Recursive};
use crate::image::write::channels::{ChannelsWriter, WritableChannels};
use crate::image::{Layer, Layers};
use crate::meta::attribute::TileDescription;
use crate::meta::header::{Header, ImageAttributes};
use crate::meta::{compute_chunk_count, Headers};
...
// impl for smallvec
impl<'slf, Channels: 'slf> WritableLayers<'slf> for Layers<Channels>
where
...
fn slice_create_writer<'slf, Channels: 'slf + WritableChannels<'slf>>(
    slice: &'slf [Layer<Channels>],
    headers: &[Header],
) -> AllLayersWriter<Channels::Writer> {
    AllLayersWriter {
        layers: slice
            .iter()
            .zip(headers.chunks_exact(1)) // TODO no array-vs-first
...
impl<'slf, Channels: WritableChannels<'slf>> WritableLayers<'slf> for Layer<Channels> {
    fn infer_headers(&self, image_attributes: &ImageAttributes) -> Headers {
        ...
        let chunk_count = compute_chunk_count(self.encoding.compression, self.size, blocks);
        ...
        let header = Header {
            ...
            deep: false, // FlatSamples are not deep data
...
impl<C> LayersWriter for LayerWriter<C>
where
    C: ChannelsWriter,
{
    fn extract_uncompressed_block(&self, headers: &[Header], block: BlockIndex) -> Vec<u8> {
        self.channels
            .extract_uncompressed_block(headers.first().expect("invalid inferred header"), block)
        // TODO no array-vs-first
    }
}
...
type RecursiveLayersWriter<InnerLayersWriter, ChannelsWriter> =
    Recursive<InnerLayersWriter, (usize, LayerWriter<ChannelsWriter>)>;
...
impl<InnerLayersWriter, Channels> LayersWriter
    for RecursiveLayersWriter<InnerLayersWriter, Channels>
where
