...
pub mod deep {
    ...
    pub fn decompress_sample_table(
        ...
        let mut decompressed = match compression {
            ...
            Compression::RLE => rle::decompress_rle_raw(compressed, expected_bytes, pedantic)?,
            Compression::ZIP1 => zip::decompress_zip_raw(compressed, expected_bytes)?,
            _ => {
                return Err(Error::unsupported(format!(
                    "compression {} not supported for deep data",
                    compression
                )))
            }
        };
    ...
    pub fn compress_sample_data(compression: Compression, data: &[u8]) -> Result<Vec<u8>> {
    ...
    mod test {
        ...
        fn roundtrip_sample_table_rle() {
            ...
            let decompressed =
                decompress_sample_table(Compression::RLE, &compressed, 5, 2, true).unwrap();
        ...
        fn roundtrip_sample_table_zip() {
            ...
            let decompressed =
                decompress_sample_table(Compression::ZIP1, &compressed, 5, 2, true).unwrap();
            assert_eq!(counts, decompressed);
        }
        ...
        fn roundtrip_sample_table_uncompressed() {
            ...
            let decompressed =
                decompress_sample_table(Compression::Uncompressed, &compressed, 2, 2, true)
                    .unwrap();
            assert_eq!(counts, decompressed);
        }
        ...
        fn roundtrip_sample_data_rle() {
            ...
            let decompressed =
                decompress_sample_data(Compression::RLE, &compressed, data.len(), true).unwrap();
