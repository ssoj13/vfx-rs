...
pub fn encode(
    ...
    if is_14_bit(max_value) {
        encode_14_or_16_bit(buffer, count, size, true)
    } else {
        encode_14_or_16_bit(buffer, count, size, false)
    }
}
...
pub fn encode_14_or_16_bit(
    ...
    let encode = if is_14_bit {
        ...
        encode_16bit
    }; // assume inlining and constant propagation
    ...
    while p2 <= count {
        ...
        while position_y <= end_y {
            // TODO: for py in (index..ey).nth(offset_2.0)
...
pub fn decode(
    ...
    if is_14_bit(max_value) {
        decode_14_or_16_bit(buffer, count, size, true)
    } else {
        decode_14_or_16_bit(buffer, count, size, false)
    }
}
...
mod test {
    ...
    fn roundtrip_14bit_image() {
        let data: [u16; 6 * 4] = [
            13, 54, 3, 123, 423, 53, 1, 23, 23, 515, 513, 43, 16374, 16381, 16284, 3, 2, 1, 0, 0,
            0, 4, 3, 0,
        ];
    ...
    fn roundtrip_16bit_image() {
        let data: [u16; 6 * 4] = [
            13, 54, 3, 123, 423, 53, 1, 23, 23, 515, 513, 43, 16385, 56384, 18384, 36384, 2, 1, 0,
            0, 0, 4, 3, 0,
        ];
