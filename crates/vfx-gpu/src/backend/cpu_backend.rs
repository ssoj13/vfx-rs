...
use super::gpu_primitives::{ImageHandle, GpuPrimitives};
...
impl GpuPrimitives for CpuPrimitives {
    ...
    fn exec_lut1d(&self, src: &Self::Handle, dst: &mut Self::Handle,
        ...
        dst.data.par_chunks_mut(c as usize)
            .zip(src.data.par_chunks(c as usize))
            .for_each(|(out, inp)| {
                for ch in 0..(c as usize).min(lut_channels as usize) {
...
// Extend ImageHandle with AsAny
impl dyn ImageHandle {
    fn as_any(&self) -> &dyn std::any::Any {
        // This is a workaround - in real code we'd use a proper trait object
        unsafe { &*(self as *const dyn ImageHandle as *const CpuImage) }
    }
    
    fn as_any_mut(&mut self) -> &mut dyn std::any::Any {
        unsafe { &mut *(self as *mut dyn ImageHandle as *mut CpuImage) }
    }
}
